...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["feed_back"])) {
    ...
    $insert_code = "INSERT INTO `blog_db`.`feedbacks` (`name`, `email`, `blogTopic`, `feedbackRating`, `comment`, `dt`) VALUES ('$name', '$email', '$blogTopic', '$feedbackRating', '$comment', current_timestamp())";
    $run_query = mysqli_query($conn, $insert_code);
    if ($run_query) {
        $subject = "Blog Feedback";
        $message = "Feedback on blog topic '' $blogTopic '' Rating - $feedbackRating     Additional Comment: $comment";
